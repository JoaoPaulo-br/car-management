<template>
  <div class="login-container">
    <el-form ref="signupForm" :model="signupForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div class="tips">
        <a style="margin-right:20px; float: right;" href="/#/login">Login</a>
      </div>
...
<script>
import { validUsername, validEmail, validMobileNumber } from '@/utils/validate'
  ...
  methods: {
    ...
    handleSignup() {
      this.$refs.signupForm.validate(valid => {
        if (valid) {
          userSignUp(this.signupForm, (error, response) => {
            this.loading = false
            if (!error) {
              this.$router.push({ path: this.redirect || '/' })
            } else {
              this.$message('Email or phone number are already in use')
            }
          })
        } else {
          return false
